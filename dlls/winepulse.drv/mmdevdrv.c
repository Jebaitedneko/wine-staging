...
static void pulse_probe_settings(pa_mainloop *ml, pa_context *ctx, int render, WAVEFORMATEXTENSIBLE *fmt) {
    ...
    if(env_usec) {
        ...
        printf("%s:%d :: Using %d usecs for attr.tlength and attr.fragsize (STAGING_AUDIO_USEC).\n", __func__, __LINE__, val);
    }
    attr.minreq = -1;
    attr.prebuf = -1;
    ...
    if(env_minp) {
        ...
        MinimumPeriod = val;
        printf("%s:%d :: Set MinimumPeriod to %d (STAGING_AUDIO_MINP).\n", __func__, __LINE__, val);
    }

    if(env_defp) {
        int val = atoi(env_defp);
        DefaultPeriod = val;
        printf("%s:%d :: Set DefaultPeriod to %d (STAGING_AUDIO_DEFP).\n", __func__, __LINE__, val);
    }
...
static HRESULT pulse_stream_connect(ACImpl *This, UINT32 period_bytes) {
    ...
    if (This->dataflow == eRender)
        ...
        PA_STREAM_START_CORKED|PA_STREAM_START_UNMUTED|PA_STREAM_AUTO_TIMING_UPDATE|PA_STREAM_INTERPOLATE_TIMING|PA_STREAM_EARLY_REQUESTS|moving, NULL, NULL);
    else
        ret = pa_stream_connect_record(This->stream, NULL, &attr,
        PA_STREAM_START_CORKED|PA_STREAM_START_UNMUTED|PA_STREAM_AUTO_TIMING_UPDATE|PA_STREAM_INTERPOLATE_TIMING|PA_STREAM_EARLY_REQUESTS|moving);
    if (ret < 0) {
        WARN("Returns %i\n", ret);
        return AUDCLNT_E_ENDPOINT_CREATE_FAILED;
    }
        ...
        const GUID *sessionguid)
{
    ...
    HRESULT hr = S_OK;
    UINT period_bytes;
    const char *env_dura = getenv("STAGING_AUDIO_DURA");
    ...
    if(env_dura) {
        int val = atoi(env_dura);
        duration = val;
        printf("%s:%d :: Set duration to %d (STAGING_AUDIO_DURA).\n", __func__, __LINE__, val);
    }
        ...
        REFERENCE_TIME *latency)
{
    ...
    REFERENCE_TIME lat;
    HRESULT hr;
    const char *env_slat = getenv("STAGING_AUDIO_SLAT");
    ...
    *latency = 10000000;
    ...
    if(env_slat) {
        int val = atoi(env_slat);
        *latency = val;
        printf("%s:%d :: Set *latency to %d (STAGING_AUDIO_SLAT).\n", __func__, __LINE__, val);
    }
